...
def precompute_all(product: str = "Motor") -> Path | None:
    ...
    In the unified Streamlit app, data is loaded from Power BI via session
    state.  Use ``run_precompute(df_motor)`` directly instead.
    """
    import streamlit as st

    df_motor = st.session_state.get("df_motor")
    if df_motor is None or df_motor.empty:
        return None
    return run_precompute(df_motor)
